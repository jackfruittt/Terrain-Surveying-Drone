      ...
      sensor_node_(nullptr)  // Initialise to nullptr
{
    ...
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/drone/gt_odom", 10, std::bind(&DroneNode::odomCallback, this, std::placeholders::_1));

    goals_sub_ = this->create_subscription<geometry_msgs::msg::PoseArray>(
        "/mission/goals", 10, std::bind(&DroneNode::goalCallback, this, std::placeholders::_1));
    ...
    takeoff_pub_ = this->create_publisher<std_msgs::msg::Empty>("/drone/takeoff", 10);
    land_pub_ = this->create_publisher<std_msgs::msg::Empty>("/drone/landing", 10);
    waypoints_pub_ = this->create_publisher<geometry_msgs::msg::PoseArray>("/mission/path", 10);
...
bool DroneNode::validateAllGoals(VehicleData& vehicle)
{
    ...
    const double MAX_X = 20.0;  // -20 to +20
    const double MAX_Y = 20.0;  // -20 to +20
    const double MAX_Z = 20.0;  // 0 to +20
...
void DroneNode::timerCallback() {
    ...
    if (!current_goals.empty()) {
        ...
        RCLCPP_DEBUG(this->get_logger(), "Published waypoints and markers for %zu goals", current_goals.size());
    }
